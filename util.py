...
def prediction(net, sess, list_sequences, list_labels, size_descriptors, epochs_completed):
    ...
    for sequences, labels in zip(list_sequences, list_labels):
        state_1_c, state_1_h = np.zeros((1, 512)), np.zeros((1, 512))
        state_2_c, state_2_h = np.zeros((1, 256)), np.zeros((1, 256))
    ...
    with open(config.path_to_save+"valid.txt", 'a') as f:
        ...
        f.write("final_final_loss_last {}".format(final_loss_last) + "\n")
        f.write("------------------------------------------\n")
        f.write("final_accuracy_mean {}".format(final_accuracy_mean)+"\n")
        f.write("final_final_loss_mean {}".format(final_loss_mean) + "\n\n")

    return final_accuracy_last, final_loss_last
...
def print_test(net, sess, list_sequences, list_labels, size_descriptors):
    ...
    for sequences, labels in zip(list_sequences, list_labels):
        ...
        for i in range(0, len(sequences) // len_clip):
            ...
            accuracy, predictions, loss, states = net.prediction(sess, clip, label, state_1_c, state_1_h, state_2_c, state_2_h)
            print('predictions : ')
...
def write_summary(net, sess, writer, epoch_completed, accuracy_last, loss_last):
    summary = net.get_summaries(sess, accuracy_last, loss_last)
