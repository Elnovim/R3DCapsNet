...
def sequence_to_list(input_tensor, size_seqs, max_length_sequences):
    mask = tf.sequence_mask(size_seqs, max_length_sequences)
...
def bias_variable(shape, stddev):
  initial = tf.constant(stddev, shape=shape, dtype=tf.float32)
  return tf.Variable(initial, name ='biases')


def linear_layer(input, w_shape, w_stddev, b_stddev, activation = None, dropout=None):
    W = weight_variable(w_shape, w_stddev)
    b = bias_variable([w_shape[1]], b_stddev)
