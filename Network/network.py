...
class NetworkModel:
    ...
    def __init__(self, base_learning_rate, n_classes, size_descriptors, scope_name, mode):
        with tf.name_scope(scope_name):
            with tf.name_scope('inputs'):
                ...
                self._state_1_c = tf.placeholder(tf.float32, shape=[None, 1024])
                self._state_1_h = tf.placeholder(tf.float32, shape=[None, 1024])
                self._state_2_c = tf.placeholder(tf.float32, shape=[None, 512])
                self._state_2_h = tf.placeholder(tf.float32, shape=[None, 512])
            ...
            with tf.name_scope('3DCapsule'):
                caps1 = create_prim_conv3d_caps(p3, 32, kernel_size=[3, 9, 9], strides=[1, 1, 1], padding="VALID", name="caps1")

                caps2 = create_conv3d_caps(caps1, 32, kernel_size=[3, 5, 5], strides=[1, 2, 2], padding="VALID", name="caps2", route_mean=True)
            ...
            pred_caps_poses = caps3[0]
            batch_size = tf.shape(pred_caps_poses)[0]
            _, n_classes, dim = pred_caps_poses.get_shape()
            n_classes, dim = map(int, [n_classes, dim])

            self._y_pred = tf.cast(tf.argmax(input=self._y, axis=1), tf.int32)
            vec_to_use = tf.cond(self._is_train, lambda: self._y_pred, lambda: self.predictions)
            ...
            with tf.name_scope('LSTM'):
                lstm_output, self._states = stacked_LSTM(lstm_input,
                                                         [1024, 512],
                                                         rnn_tuple_state=(
                                                             tf.contrib.rnn.LSTMStateTuple(self._state_1_c, self._state_1_h),
                                                             tf.contrib.rnn.LSTMStateTuple(self._state_2_c, self._state_2_h)
                                                         ),
                                                         list_length_sequence=self._lengths_sequence,
                                                         dropout=self._dropout,
                                                         name_scope=scope_name+'LSTM')
    ...
    def prediction(self, sess, batch_x, batch_y, state_1_c, state_1_h, state_2_c, state_2_h, m):
        return sess.run([self._accuracy, self._prediction_to_accuracy, self._caps_loss, self._pred_loss, self._final_loss, self._states], feed_dict={
            ...
            self._state_1_c: state_1_c,
            self._state_1_h: state_1_h,
